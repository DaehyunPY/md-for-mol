...
void denka::keisan(int n_th)
	...
	for(i = 0; i <= n_step; i++) {
		...
		if(CT_para == 99) {
			...
			if(t != 0)
				...
				for(k = 0; k != 1 && k != 2; k++)
					{
					charge_next[1] = charge_next[1] - charge_next[k] * (charge_next[1]*charge_next[1] / (charge_next[1]*charge_next[1] + charge_next[2]*charge_next[2]));
					charge_next[2] = charge_next[2] - charge_next[k] * (charge_next[2]*charge_next[2] / (charge_next[1]*charge_next[1] + charge_next[2]*charge_next[2]));
					}
				}
		...
		else if(CT_para == 92)
		{
            ...
            for(k = 0; k < PN; k++) {
                ...
                + velocity[k][1]*velocity[k][1]+velocity[k][2]*velocity[k][2]);
	...
	if(refmomentum<fabs(kyori(px,initialP[0],py,initialP[1],pz,initialP[2])/minmomentum))  //大きい方をrefmomentumに採用
		{
		...
		case 'L'	:
			for (seisu=0;seisu<PN;seisu++)  
				...
				if(seisu-i) 
					...
					+12*pow(SIGMA/nagasa,12)))*(position[i][j]-position[seisu][j])
...
int denka::get_data(char *input_file)
	...
	for(i=1;i<PN;i++)
		...
		if(flag==0)
			{
			valid_label[num_label] = label[i]; num_label=num_label+1;
			}
	...
	// 	if(charge_dest[i] == -1)
	// 		{
	// 		random_charge[num_random_atom]=i;
	// 		num_random_atom = num_random_atom + 1;
	// 		}
	// 	if(charge_dest[i] != -1)
	// 		{
	// 		num_fixed_atom = num_fixed_atom +1;
	// 		}
	// 	}
